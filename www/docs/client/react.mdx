    ...
    <div>
      <div>
        <RenderMessages
          ...
          // Rendering a human message. Theres no shell/content separation here because human messages only have a single part
...
function HumanMessage({ message }: { message: ChatAIMessage<MyAgentType> }) {
