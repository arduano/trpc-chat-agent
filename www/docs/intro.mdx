...
Frontend adapters only require 3 arguments:
...
The frontend adapter mainly provides a list of messages to render, and some helper functions to interact with those messages. All state is handled automatically.

Here is a simple example of the React frontend adapter:
...
<div style={{ display: 'flex', gap: '10px', marginTop: '20px' }}>
  <Button asChild variant="secondary">
    <Link to="/docs/frontend/react">React</Link>
  </Button>
  <Button variant="outline" disabled>
    <div>Vue</div>
  </Button>
